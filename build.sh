...
if ! grep -Fxq "gen" /opt/build/repo/_config.yml; then
...
#################################################################
curl https://raw.githubusercontent.com/opengovsg/isomer-build/master-no-version-check/overrides/netlify.toml -o /opt/build/repo/netlify.toml

###################################################################
# Obtain config override file to enforce plugins and remote theme #
###################################################################
curl https://raw.githubusercontent.com/opengovsg/isomer-build/master-no-version-check/overrides/_config-override.yml -o /opt/build/repo/_config-override.yml
